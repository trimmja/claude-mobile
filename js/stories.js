...
function conditionsMet(conditions, state) {
  ...
  // Dynamic stage conditions: stageMin_kenji, stageMax_yuki, etc.
  for (const [key, val] of Object.entries(conditions)) {
    const minMatch = key.match(/^stageMin_(\w+)$/);
    const maxMatch = key.match(/^stageMax_(\w+)$/);
    if (minMatch && (state.npcs[minMatch[1]]?.stage ?? -1) < val) return false;
    if (maxMatch && (state.npcs[maxMatch[1]]?.stage ?? 99) > val) return false;
  }
